...
/**
 * ReadmillApi
 * A PHP-based Readmill client library with a focus on simplicity and ease of integration
 * 
 ...
 * Readmill API, as well as the necessary functionality for acquiring an 
 * access token for a user via Readmill web authentication
 */

class ReadmillApiException extends Exception {}

class ReadmillApi {

	/** @var String $BaseUrl The base url for the readmill API */
	private $BaseUrl = "http://api.readmill.com/";
	/** @var String $AuthUrl The url for obtaining the auth access code */
	private $AuthUrl = "http://readmill.com/oauth/authorize";
	/** @var String $TokenUrl The url for obtaining an auth token */
	private $TokenUrl = "http://readmill.com/oauth/token";
	 ...
	 * @param String $endpoint A particular endpoint of the Readmill API
    ...
    public function getResponseFromJsonString($json) {
        ...
        if ( !isset( $json->response ) ) {
            throw new ReadmillApiException( 'Invalid response' );
        }
	 ...
	 * packaged with ReadmillApi to facilitate locality searches.
	 ...
	 * @param String $token A Readmill user auth_token
	 */
	...
	public function AuthenticationLink($redirect='',$noexpire=false){
		...
		if ($noexpire){
		$params = array("client_id"=>$this->ClientID,"response_type"=>"code","redirect_uri"=>$redirect,"scope"=>"non-expiring");
		} else {
	...
	/**
	 * GetToken
	 * Performs a request to Readmill for a user token, and returns the token, while also storing it
	 * locally for use in private requests
	 * @param $code The 'code' parameter provided by the Readmill webauth callback redirect
	...
	public function GetToken($code,$redirect=''){
		...
		$params = array("client_id"=>$this->ClientID,
						...
						"code"=>$code);
		$result = $this->POST($this->TokenUrl,$params);
